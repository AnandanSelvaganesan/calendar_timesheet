...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
				...
				'<div class="fc-content-skeleton">' +
					'<table>' +
						(this.numbersVisible ?
							...
							(this.numbersVisible ?
									'<tfoot>' +
										this.renderBottomRowTrHtml(row) +
									'</tfoot>' :
									''
									)+	
	...
	/* Grid bottom row Rendering
	------------------------------------------------------------------------------------------------------------------*/


	renderBottomRowTrHtml: function(row) {
		return '' +
			'<tr>' +
				(this.isRTL ? '' : this.renderBottomRowIntroHtml(row)) +
				this.renderBottomRowCellsHtml(row) +
				(this.isRTL ? this.renderBottomRowIntroHtml(row) : '') +
			'</tr>';
	},


	renderBottomRowIntroHtml: function(row) {
		return this.renderIntroHtml();
	},


	renderBottomRowCellsHtml: function(row) {
		var htmls = [];
		var col, date;

		for (col = 0; col < this.colCnt; col++) {
			date = this.getCellDate(row, col);
			htmls.push(this.renderBottomRowCellHtml(date));
		}
	...
	// Generates the HTML for the <td>s of the "number" row in the DayGrid's content skeleton.
	// The number row will only exist if either day numbers or week numbers are turned on.
	renderBottomRowCellHtml: function(date) {
		var html = '';
		var classes;
		var weekCalcFirstDoW;
